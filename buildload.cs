function loadBuild(%offset, %build, %brickgroup) {
	if (!isFile("Add-ons/Server_Board_Games/saves/" @ %build @ ".bls")) {
		echo("Board Game Error: Load file not found");
		return;
	}

	%file = new FileObject();
	%tempGroup = new SimGroup();
	%file.openForRead("Add-ons/Server_Board_Games/saves/" @ %build @ ".bls");
	%file.readLine();
	%cnt = %file.readLine();
